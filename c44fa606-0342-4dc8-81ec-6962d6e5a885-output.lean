...
This project request had uuid: c44fa606-0342-4dc8-81ec-6962d6e5a885
...
/-
I have implemented the "AdjoinRoot Transfer" layer (Layer 3) of the Frobenius Orientation Theorem proof.
This includes:
1.  `algebra_zmod_quotient`: Establishing the `Algebra (ZMod N) (Okcyc q ⧸ P)` instance.
2.  `phi_hom`: Constructing the ring homomorphism $\varphi : K \to \mathcal{O}_K/P$.
3.  `phi_hom_injective`: Proving $\varphi$ is injective.
4.  `phi_alpha`, `phi_beta`, `phi_gamma`: Proving $\varphi$ maps $\alpha, \beta, \gamma$ in $K$ to their counterparts in $\mathcal{O}_K/P$.
5.  `frobenius_orientation`: The main theorem, proved by transferring the result from the "Dream Theorem" in $\mathcal{O}_K/P$ back to $K$ using $\varphi$.

I followed "Approach A" by adding the `h_beta_rel` hypothesis (the beta relation in the cyclotomic field) to the `frobenius_orientation` theorem, as suggested in the blueprint. I also used `⁻¹` instead of `^{-1}` in the definition of `beta` within the theorem statement to ensure definitional equality with the helper definitions, which allowed the proof to go through smoothly.
-/
...
/-
Construct an Algebra (ZMod N) instance for the quotient ring Ok/P.
-/
noncomputable def algebra_zmod_quotient (q N : ℕ)
  [Fact (Nat.Prime q)] [Fact (Nat.Prime N)]
  (P : Ideal (Okcyc q)) [P.IsMaximal]
  (hP : P.LiesOver (Ideal.span {(N : ℤ)})) :
  Algebra (ZMod N) (Okcyc q ⧸ P) := by
  have h_char := ringChar_Ok_P q N P hP
  haveI : CharP (Okcyc q ⧸ P) N := by
    rw [← h_char]
    exact ringChar.charP (Okcyc q ⧸ P)
  exact ZMod.algebra (Okcyc q ⧸ P) N

/-
Construct the homomorphism phi from K to Ok/P.
-/
noncomputable def phi_hom (q : ℕ) (a : ℤ) (N : ℕ)
  [Fact (Nat.Prime q)] [Fact (Nat.Prime N)]
  (h_eq : 4 * (q : ℚ) = (a : ℚ)^2 + 27)
  (h_period : alpha_cyclotomic q ^ 3 = (q : Kcyc q) * alpha_cyclotomic q + (q : Kcyc q))
  (P : Ideal (Okcyc q)) [P.IsMaximal]
  (hP : P.LiesOver (Ideal.span {(N : ℤ)}))
  [Algebra (ZMod N) (Okcyc q ⧸ P)] :
  AdjoinRoot (f_mod_N q N) →ₐ[ZMod N] (Okcyc q ⧸ P) :=
  AdjoinRoot.liftHom (f_mod_N q N)
    (Ideal.Quotient.mk P (alpha_int_cyc q))
    (roots_satisfy_f_in_quotient_v2 q a N h_eq h_period P hP).1

/-
Prove that phi_hom is injective.
-/
lemma phi_hom_injective (q : ℕ) (a : ℤ) (N : ℕ)
  [Fact (Nat.Prime q)] [Fact (Nat.Prime N)]
  (h_eq : 4 * (q : ℚ) = (a : ℚ)^2 + 27)
  (h_period : alpha_cyclotomic q ^ 3 = (q : Kcyc q) * alpha_cyclotomic q + (q : Kcyc q))
  (P : Ideal (Okcyc q)) [P.IsMaximal]
  (hP : P.LiesOver (Ideal.span {(N : ℤ)}))
  [Algebra (ZMod N) (Okcyc q ⧸ P)]
  [Fact (Irreducible (f_mod_N q N))] :
  Function.Injective (phi_hom q a N h_eq h_period P hP) := by
    apply RingHom.injective

/-
Prove that phi maps the root of f to alpha_bar.
-/
lemma phi_alpha (q : ℕ) (a : ℤ) (N : ℕ)
  [Fact (Nat.Prime q)] [Fact (Nat.Prime N)]
  (h_eq : 4 * (q : ℚ) = (a : ℚ)^2 + 27)
  (h_period : alpha_cyclotomic q ^ 3 = (q : Kcyc q) * alpha_cyclotomic q + (q : Kcyc q))
  (P : Ideal (Okcyc q)) [P.IsMaximal]
  (hP : P.LiesOver (Ideal.span {(N : ℤ)}))
  [Algebra (ZMod N) (Okcyc q ⧸ P)] :
  phi_hom q a N h_eq h_period P hP (AdjoinRoot.root (f_mod_N q N))
    = Ideal.Quotient.mk P (alpha_int_cyc q) := by
  unfold phi_hom
...
Define beta in K.
-/
noncomputable def beta_K (q : ℕ) (a : ℤ) (N : ℕ)
  [Fact (Nat.Prime N)] [Fact (Irreducible (f_mod_N q N))] : AdjoinRoot (f_mod_N q N) :=
  letI : Field (AdjoinRoot (f_mod_N q N)) := AdjoinRoot.instField
  let r := AdjoinRoot.root (f_mod_N q N)
  (6 * r^2 - (a + 9 : AdjoinRoot (f_mod_N q N)) * r - 4 * (q : AdjoinRoot (f_mod_N q N))) * (2 * (a : AdjoinRoot (f_mod_N q N)))⁻¹

/-
Prove that phi(beta) = beta_bar.
-/
lemma phi_beta (q : ℕ) (a : ℤ) (N : ℕ)
  [Fact (Nat.Prime q)] [Fact (Nat.Prime N)]
  (h_eq : 4 * (q : ℚ) = (a : ℚ)^2 + 27)
  (h_period : alpha_cyclotomic q ^ 3 = (q : Kcyc q) * alpha_cyclotomic q + (q : Kcyc q))
  (P : Ideal (Okcyc q)) [P.IsMaximal]
  (hP : P.LiesOver (Ideal.span {(N : ℤ)}))
  [Algebra (ZMod N) (Okcyc q ⧸ P)]
  (h_N_odd : Odd N) (h_gcd : Int.gcd a N = 1)
  [Fact (Irreducible (f_mod_N q N))]
  (h_beta_rel : 2 * (a : Kcyc q) * beta_cyclotomic q = 6 * alpha_cyclotomic q ^ 2 - (a + 9 : Kcyc q) * alpha_cyclotomic q - 4 * (q : Kcyc q)) :
  phi_hom q a N h_eq h_period P hP (beta_K q a N) = Ideal.Quotient.mk P (beta_int_cyc q) := by
    have h_beta_eq : (phi_hom q a N h_eq h_period P hP) ((2 * (a : AdjoinRoot (f_mod_N q N))) * beta_K q a N) = (Ideal.Quotient.mk P) (2 * (a : Okcyc q) * beta_int_cyc q) := by
      have h_beta_eq : (phi_hom q a N h_eq h_period P hP) ((2 * (a : AdjoinRoot (f_mod_N q N))) * beta_K q a N) = (phi_hom q a N h_eq h_period P hP) (6 * (AdjoinRoot.root (f_mod_N q N))^2 - (a + 9) * (AdjoinRoot.root (f_mod_N q N)) - 4 * (q : AdjoinRoot (f_mod_N q N))) := by
        unfold beta_K;
        have h_inv : (2 * (a : AdjoinRoot (f_mod_N q N))) * (2 * (a : AdjoinRoot (f_mod_N q N)))⁻¹ = 1 := by
          have h_inv : (2 * (a : ZMod N)) ≠ 0 := by
            intro h; have := Fact.out ( p := Nat.Prime N ) ; simp_all +decide [ ← ZMod.intCast_eq_intCast_iff ] ;
            exact absurd ( h.resolve_left ( by erw [ ZMod.natCast_eq_zero_iff ] ; exact Nat.not_dvd_of_pos_of_lt ( by decide ) ( lt_of_le_of_ne this.two_le ( Ne.symm <| by rintro rfl; contradiction ) ) ) ) ( by rw [ ZMod.intCast_zmod_eq_zero_iff_dvd ] ; exact fun h' => by have := Int.dvd_gcd h' ( dvd_refl _ ) ; simp_all +decide );
          have h_inv : IsUnit (2 * (a : AdjoinRoot (f_mod_N q N))) := by
            have h_inv : IsUnit (2 * a : ZMod N) := by
              exact IsUnit.mk0 _ h_inv;
            convert h_inv.map ( algebraMap ( ZMod N ) ( AdjoinRoot ( f_mod_N q N ) ) ) using 1;
            norm_num +zetaDelta at *;
            exact Or.inl rfl;
          field_simp;
          rw [ div_eq_mul_inv, mul_inv_cancel₀ h_inv.ne_zero ];
        grind;
      have h_beta_eq : (phi_hom q a N h_eq h_period P hP) (6 * (AdjoinRoot.root (f_mod_N q N))^2 - (a + 9) * (AdjoinRoot.root (f_mod_N q N)) - 4 * (q : AdjoinRoot (f_mod_N q N))) = 6 * (Ideal.Quotient.mk P (alpha_int_cyc q))^2 - (a + 9) * (Ideal.Quotient.mk P (alpha_int_cyc q)) - 4 * (q : Okcyc q ⧸ P) := by
        have h_beta_eq : (phi_hom q a N h_eq h_period P hP) (AdjoinRoot.root (f_mod_N q N)) = (Ideal.Quotient.mk P (alpha_int_cyc q)) := by
          exact phi_alpha q a N h_eq h_period P hP
        generalize_proofs at *; (
        convert congr_arg ( fun x : Okcyc q ⧸ P => 6 * x ^ 2 - ( a + 9 ) * x - 4 * ( q : Okcyc q ⧸ P ) ) h_beta_eq using 1 ; ring!;
        norm_num +zetaDelta at *;
        erw [ map_natCast, map_natCast, map_natCast ] ; norm_num;);
      convert h_beta_eq using 1;
      convert congr_arg ( Ideal.Quotient.mk P ) ( show ( 2 * a : Okcyc q ) * beta_int_cyc q = 6 * alpha_int_cyc q ^ 2 - ( a + 9 ) * alpha_int_cyc q - 4 * ( q : Okcyc q ) from ?_ ) using 1;
      convert h_beta_rel using 1;
      constructor <;> intro h;
      · convert h_beta_rel using 1;
      · exact?;
    have h_inv : IsUnit (2 * (a : ZMod N)) := by
      have h_inv : IsUnit (a : ZMod N) := by
        have := Int.gcd_eq_gcd_ab a N;
        replace this := congr_arg ( ( ↑ ) : ℤ → ZMod N ) this ; aesop;
      exact IsUnit.mul ( by exact isUnit_iff_ne_zero.mpr ( by erw [ Ne.eq_def, ZMod.natCast_eq_zero_iff ] ; exact Nat.not_dvd_of_pos_of_lt Nat.zero_lt_two ( lt_of_le_of_ne ( Nat.Prime.two_le Fact.out ) ( Ne.symm <| by rintro rfl; contradiction ) ) ) ) h_inv;
    have h_inv : IsUnit (phi_hom q a N h_eq h_period P hP (2 * (a : AdjoinRoot (f_mod_N q N)))) := by
      convert h_inv.map ( algebraMap ( ZMod N ) ( Okcyc q ⧸ P ) ) using 1;
      convert ( phi_hom q a N h_eq h_period P hP ).commutes _ using 1;
      norm_num [ Algebra.smul_def ];
      exact Or.inl rfl;
    have h_inv : IsUnit (phi_hom q a N h_eq h_period P hP (2 * (a : AdjoinRoot (f_mod_N q N)))) → phi_hom q a N h_eq h_period P hP (beta_K q a N) = (Ideal.Quotient.mk P) (beta_int_cyc q) := by
      intro h_inv
      have h_eq : phi_hom q a N h_eq h_period P hP (2 * (a : AdjoinRoot (f_mod_N q N))) * phi_hom q a N h_eq h_period P hP (beta_K q a N) = phi_hom q a N h_eq h_period P hP (2 * (a : AdjoinRoot (f_mod_N q N))) * (Ideal.Quotient.mk P) (beta_int_cyc q) := by
        convert h_beta_eq using 1 <;> norm_num [ mul_assoc ];
        exact Or.inl <| map_natCast _ _;
      exact h_inv.mul_left_cancel h_eq;
    exact h_inv ‹_›

/-
Check if phi_beta exists.
-/
#check phi_beta
...
Define gamma in K.
-/
noncomputable def gamma_K (q : ℕ) (a : ℤ) (N : ℕ)
  [Fact (Nat.Prime N)] [Fact (Irreducible (f_mod_N q N))] : AdjoinRoot (f_mod_N q N) :=
  - AdjoinRoot.root (f_mod_N q N) - beta_K q a N

/-
Prove that phi(gamma) = gamma_bar.
-/
lemma phi_gamma (q : ℕ) (a : ℤ) (N : ℕ)
  [Fact (Nat.Prime q)] [Fact (Nat.Prime N)]
  (h_eq : 4 * (q : ℚ) = (a : ℚ)^2 + 27)
  (h_period : alpha_cyclotomic q ^ 3 = (q : Kcyc q) * alpha_cyclotomic q + (q : Kcyc q))
  (P : Ideal (Okcyc q)) [P.IsMaximal]
  ...
  [Fact (Irreducible (f_mod_N q N))]
  (h_beta_rel : 2 * (a : Kcyc q) * beta_cyclotomic q = 6 * alpha_cyclotomic q ^ 2 - (a + 9 : Kcyc q) * alpha_cyclotomic q - 4 * (q : Kcyc q)) :
  phi_hom q a N h_eq h_period P hP (gamma_K q a N) = Ideal.Quotient.mk P (gamma_int_cyc q) := by
    -- Apply the homomorphism property of phi_hom to split the sum.
    have h_hom : (phi_hom q a N h_eq h_period P hP) (-AdjoinRoot.root (f_mod_N q N) - beta_K q a N) = -(phi_hom q a N h_eq h_period P hP (AdjoinRoot.root (f_mod_N q N))) - (phi_hom q a N h_eq h_period P hP (beta_K q a N)) := by
      rw [ map_sub, map_neg ];
    -- Substitute the known values of alpha_bar and beta_bar into the expression for gamma_bar.
    have h_gamma_bar : (Ideal.Quotient.mk P (gamma_int_cyc q)) = -(Ideal.Quotient.mk P (alpha_int_cyc q)) - (Ideal.Quotient.mk P (beta_int_cyc q)) := by
      convert congr_arg ( Ideal.Quotient.mk P ) ( show gamma_int_cyc q = -alpha_int_cyc q - beta_int_cyc q from ?_ ) using 1;
      ext; simp [gamma_int_cyc, alpha_int_cyc, beta_int_cyc];
      unfold gamma_cyclotomic alpha_cyclotomic beta_cyclotomic; ring;
    convert h_hom using 1;
    rw [ phi_alpha, phi_beta ] ; aesop;
    · exact h_N_odd;
    · exact h_gcd;
    · convert h_beta_rel using 1

/-
Check if phi_gamma exists.
-/
#check phi_gamma

/-
Prove the algebraic identity for B.
-/
lemma beta_poly_identity_proof {R : Type*} [CommRing R] (a q : R) (alpha : R)
  (h_root : alpha^3 = q * alpha + q)
  (h_a2 : a^2 = 4 * q - 27) :
  let B := 6 * alpha^2 - (a + 9) * alpha - 4 * q
  B^3 - 4 * a^2 * q * B - 8 * a^3 * q = 0 := by
...
/-
Prove the Frobenius Orientation Theorem.
-/
theorem frobenius_orientation (q : ℕ) (a : ℤ)
  [Fact (Nat.Prime q)]
  (h_eq : 4 * (q : ℚ) = (a : ℚ)^2 + 27)
  (h_period : alpha_cyclotomic q ^ 3
    = (q : Kcyc q) * alpha_cyclotomic q + (q : Kcyc q))
  (N : ℕ) [Fact (Nat.Prime N)]
  (h_N_odd : Odd N) (h_N_ne_q : N ≠ q)
  (h_gcd : Int.gcd a N = 1)
  (h_chi_s2 : chi q (g_gen q) = s2 q a)
  [Fact (Irreducible (f_mod_N q N))]
  (h_beta_rel : 2 * (a : Kcyc q) * beta_cyclotomic q = 6 * alpha_cyclotomic q ^ 2 - (a + 9 : Kcyc q) * alpha_cyclotomic q - 4 * (q : Kcyc q)) :
  let K := AdjoinRoot (f_mod_N q N)
  let alpha := AdjoinRoot.root (f_mod_N q N)
  let beta : K := (6 * alpha^2 - (a+9:K) * alpha
    - 4*(q:K)) * (2*(a:K))⁻¹
  let gamma : K := -alpha - beta
  (c_value q N = s1 q a → alpha ^ N = gamma) ∧
  (c_value q N = s2 q a → alpha ^ N = beta) := by
    obtain ⟨ P, hP₁, hP₂ ⟩ := exists_maximal_lies_over q N h_N_ne_q;
    have := dream_theorem_cyclotomic q a h_eq h_period N h_N_ne_q h_gcd P hP₂ h_chi_s2;
    convert this using 1;
    constructor <;> intro h <;> simp_all +decide [ ← eq_sub_iff_add_eq' ];
    apply And.intro;
    · intro h_eq_s1
      have h_alpha_gamma : (Ideal.Quotient.mk P (alpha_int_cyc q)) ^ N = (Ideal.Quotient.mk P (gamma_int_cyc q)) := by
        exact h.1 h_eq_s1;
      convert phi_hom_injective q a N h_eq h_period P hP₂ |> fun h => h ( show phi_hom q a N h_eq h_period P hP₂ _ = phi_hom q a N h_eq h_period P hP₂ _ from ?_ ) using 1;
      exact algebra_zmod_quotient q N P hP₂;
      convert h_alpha_gamma using 1;
      · convert congr_arg ( · ^ N ) ( phi_alpha q a N h_eq h_period P hP₂ ) using 1;
        exact map_pow _ _ _;
      · convert phi_gamma q a N h_eq h_period P hP₂ h_N_odd h_gcd h_beta_rel using 1;
        rotate_left;
        exact algebra_zmod_quotient q N P hP₂;
        unfold gamma_K; ring;
        unfold beta_K; ring;
    · intro h_eq_s2
      have h_beta_eq : (Ideal.Quotient.mk P (alpha_int_cyc q)) ^ N = (Ideal.Quotient.mk P (beta_int_cyc q)) := by
        exact h.2 h_eq_s2;
      convert phi_hom_injective q a N h_eq h_period P hP₂ |> fun h => h ( show phi_hom q a N h_eq h_period P hP₂ _ = phi_hom q a N h_eq h_period P hP₂ _ from ?_ ) using 1;
      exact algebra_zmod_quotient q N P hP₂;
      convert h_beta_eq using 1;
      · convert congr_arg ( · ^ N ) ( phi_alpha q a N h_eq h_period P hP₂ ) using 1;
        exact map_pow _ _ _;
      · convert phi_beta q a N h_eq h_period P hP₂ h_N_odd h_gcd h_beta_rel using 1;
        convert rfl using 2;
        unfold beta_K; ring;