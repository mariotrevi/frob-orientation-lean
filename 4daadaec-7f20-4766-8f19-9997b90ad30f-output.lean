...
This project request had uuid: 4daadaec-7f20-4766-8f19-9997b90ad30f
...
/-
We have formalized the generator-independence of the period equation for the cubic period differences.
We defined generator-parameterized versions of the cosets (`C_coset_g`), Gaussian periods (`eta_period_g`), raw period differences (`alpha_raw_g`, etc.), and normalized period differences (`alpha_g`, etc.).
We proved that under a change of generator $g' = g^k$:
- If $k \equiv 1 \pmod 3$, the cosets and periods are unchanged.
- If $k \equiv 2 \pmod 3$, the cosets are permuted ($C_1 \leftrightarrow C_2$), causing the raw periods to transform as $\alpha \mapsto -\gamma$, $\beta \mapsto -\beta$, $\gamma \mapsto -\alpha$.
We showed that the normalization factor $\epsilon(g)$ transforms such that the normalized periods are simply permuted in the $k \equiv 2 \pmod 3$ case.
Finally, we proved `generator_independence`, stating that if $\alpha(g)$ satisfies the cubic equation $X^3 - qX - q = 0$, then $\alpha(g')$, $\beta(g')$, and $\gamma(g')$ also satisfy it for any other generator $g'$.
We also linked these results to the original fixed-generator definitions.
-/
...
/-
Definitions of generator-dependent cosets and Gaussian periods.
-/
def C_coset_g (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) (i : ZMod 3) : Finset (ZMod q) :=
  (H_finset q).image (fun x => ((g ^ i.val * x) : ZMod q))

noncomputable def eta_period_g (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) (i : ZMod 3) : CyclotomicField q ℚ :=
  ∑ t ∈ C_coset_g q g i, (zeta_q q) ^ t.val

/-
Definitions of raw period differences.
-/
noncomputable def alpha_raw_g (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) : CyclotomicField q ℚ :=
  eta_period_g q g 0 - eta_period_g q g 1

noncomputable def beta_raw_g (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) : CyclotomicField q ℚ :=
  eta_period_g q g 1 - eta_period_g q g 2

noncomputable def gamma_raw_g (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) : CyclotomicField q ℚ :=
  eta_period_g q g 2 - eta_period_g q g 0

/-
Definitions of the normalization factor epsilon and the normalized periods.
-/
noncomputable def epsilon_g (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) : ℤ :=
  if alpha_raw_g q g * beta_raw_g q g * gamma_raw_g q g = (q : CyclotomicField q ℚ) then 1 else -1

noncomputable def alpha_g (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) : CyclotomicField q ℚ :=
  (epsilon_g q g : ℚ) • alpha_raw_g q g

noncomputable def beta_g (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) : CyclotomicField q ℚ :=
  (epsilon_g q g : ℚ) • beta_raw_g q g

noncomputable def gamma_g (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) : CyclotomicField q ℚ :=
  (epsilon_g q g : ℚ) • gamma_raw_g q g

/-
Lemma: If g and g' are generators, then g' is a power of g with exponent k not divisible by 3.
-/
lemma cosets_change (q : ℕ) [Fact (Nat.Prime q)] (h_div : 3 ∣ q - 1) (g g' : (ZMod q)ˣ)
  (hg : orderOf g = q - 1) (hg' : orderOf g' = q - 1) :
  ∃ k : ℕ, g' = g ^ k ∧ (k % 3 = 1 ∨ k % 3 = 2) := by
    obtain ⟨k, hk⟩ : ∃ k : ℕ, g' = g ^ k := by
      have h_gen : Subgroup.zpowers g = ⊤ := by
        refine' Subgroup.eq_top_of_card_eq _ _ ; simp_all +decide [ orderOf_eq_card_of_forall_mem_zpowers ];
        rw [ Fintype.card_zpowers, hg, Nat.totient_prime Fact.out ];
      rw [ Subgroup.eq_top_iff' ] at h_gen;
      obtain ⟨ k, hk ⟩ := h_gen g';
      use Int.toNat ( k % orderOf g ) ; simp +decide [ ← hk, ← zpow_natCast, Int.toNat_of_nonneg ( Int.emod_nonneg _ <| show ( orderOf g : ℤ ) ≠ 0 from mod_cast hg.symm ▸ Nat.cast_ne_zero.mpr ( Nat.sub_ne_zero_of_lt <| Nat.Prime.one_lt Fact.out ) ), zpow_mod_orderOf ] ;
    refine' ⟨ k % ( q - 1 ), _, _ ⟩;
    · rw [ hk, ← Nat.mod_add_div k ( q - 1 ), pow_add, pow_mul ] ; simp +decide [ ← hg, pow_orderOf_eq_one ];
    · have h_mod : orderOf (g ^ k) = (q - 1) / Nat.gcd k (q - 1) := by
        rw [ orderOf_pow, hg ];
        rw [ Nat.gcd_comm ];
      have h_gcd : Nat.gcd k (q - 1) = 1 := by
        have h_gcd : (q - 1) / Nat.gcd k (q - 1) = q - 1 := by
          grind;
        nlinarith [ Nat.div_mul_cancel ( Nat.gcd_dvd_right k ( q - 1 ) ), Nat.sub_pos_of_lt ( show 1 < q from Fact.out ) ];
      have h_mod : k % 3 ≠ 0 := by
        exact fun h => by have := Nat.dvd_gcd ( Nat.dvd_of_mod_eq_zero h ) h_div; simp_all +decide ;
      rw [ Nat.mod_mod_of_dvd _ h_div ] ; have := Nat.mod_lt k zero_lt_three; interval_cases k % 3 <;> trivial;

/-
Lemma: If k is congruent to 1 modulo 3, then the cosets for g^k are the same as for g.
-/
lemma coset_g_eq_of_pow_one (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) (k : ℕ)
  (hk : k % 3 = 1) :
  ∀ i : ZMod 3, C_coset_g q (g ^ k) i = C_coset_g q g i := by
    -- Since $k \equiv 1 \pmod{3}$, we have $g^k = g^{3m + 1} = (g^3)^m \cdot g = g$ for some integer $m$.
    obtain ⟨m, hm⟩ : ∃ m : ℕ, k = 3 * m + 1 := by
      exact ⟨ k / 3, by rw [ ← hk, Nat.div_add_mod ] ⟩;
    simp +decide [ hm, pow_add, pow_mul ];
    -- Since $g^3$ is in the subgroup $H$, multiplying by $g^{3m}$ does not change the coset.
    have h_coset_eq : ∀ i : ZMod 3, C_coset_g q ((g ^ 3) ^ m * g) i = C_coset_g q g i := by
      intro i
      have h_subgroup : (g ^ 3) ^ m ∈ H_finset q := by
        simp +decide [ H_finset, pow_right_comm ];
        exact ⟨ g ^ m, by simp +decide [ pow_right_comm ] ⟩
      unfold C_coset_g; simp +decide [ Finset.ext_iff ] ;
      intro a; constructor <;> rintro ⟨ x, hx, rfl ⟩ <;> simp_all +decide [ mul_pow, pow_right_comm ] ;
      · use (g ^ m) ^ (3 * i.val) * x; simp_all +decide [ pow_mul, mul_assoc, mul_left_comm, mul_comm ] ;
        unfold H_finset at *; aesop;
      · use ((g ^ m) ^ 3)⁻¹ ^ i.val * x; simp_all +decide [ mul_assoc, mul_comm, mul_left_comm ] ;
        unfold H_finset at *; aesop;
    exact h_coset_eq

/-
Lemma: If k is congruent to 2 modulo 3, then the cosets for g^k are permuted: C_0->C_0, C_1->C_2, C_2->C_1.
-/
lemma coset_g_eq_of_pow_two (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) (k : ℕ)
  (hk : k % 3 = 2) :
  C_coset_g q (g ^ k) 0 = C_coset_g q g 0 ∧
  C_coset_g q (g ^ k) 1 = C_coset_g q g 2 ∧
  C_coset_g q (g ^ k) 2 = C_coset_g q g 1 := by
    -- Since $k \equiv 2 \pmod{3}$, we have $g^k = g^{3m+2}$ for some $m$.
    obtain ⟨m, rfl⟩ : ∃ m, k = 3 * m + 2 := by
      exact ⟨ k / 3, by rw [ ← hk, Nat.div_add_mod ] ⟩;
    -- Since $g^{3m+2} = g^2 \cdot (g^3)^m$ and $(g^3)^m \in H$, we have $g^{3m+2} H = g^2 H$.
    have h_coset_g2 : ∀ i : ZMod 3, C_coset_g q (g ^ (3 * m + 2)) i = C_coset_g q (g ^ 2) i := by
      intro i
      simp [C_coset_g];
      ext; simp [pow_add, pow_mul];
      -- Since $g^3 \in H$, we have $(g^3)^{im} \in H$.
      have h_g3_im_in_H : ∀ m : ℕ, (g ^ 3 : (ZMod q)ˣ) ^ m ∈ H_finset q := by
        intro m; exact (by
        simp +decide [ H_finset, pow_right_comm ];
        exact ⟨ g ^ m, rfl ⟩);
      constructor <;> rintro ⟨ a, ha, rfl ⟩ <;> simp_all +decide [ mul_pow, mul_assoc, mul_comm, mul_left_comm ] ;
      · use a * (g ^ 3) ^ (m * i.val);
        simp_all +decide [ mul_assoc, mul_comm, mul_left_comm, pow_mul ];
        have h_subgroup : ∀ x y : (ZMod q)ˣ, x ∈ H_finset q → y ∈ H_finset q → x * y ∈ H_finset q := by
          unfold H_finset; aesop;
        generalize_proofs at *; (
        exact h_subgroup _ _ ha ( by simpa using h_g3_im_in_H ( m * i.val ) ) |> fun h => by simpa [ pow_mul ] using h;);
      · refine' ⟨ a * ( ( g ^ 3 ) ^ m ) ⁻¹ ^ i.val, _, _ ⟩ <;> simp_all +decide [ mul_assoc, mul_comm, mul_left_comm, pow_mul ];
        have h_g3_im_in_H : ∀ m : ℕ, ∀ i : ZMod 3, ((g ^ 3 : (ZMod q)ˣ) ^ m) ^ i.val ∈ H_finset q := by
          intro m i; specialize h_g3_im_in_H ( m * i.val ) ; simp_all +decide [ pow_mul ] ;
        have h_g3_im_in_H : ∀ m : ℕ, ∀ i : ZMod 3, (((g ^ 3 : (ZMod q)ˣ) ^ m) ^ i.val)⁻¹ ∈ H_finset q := by
          simp_all +decide [ H_finset ];
        have h_g3_im_in_H : ∀ x y : (ZMod q)ˣ, x ∈ H_finset q → y ∈ H_finset q → x * y ∈ H_finset q := by
          unfold H_finset; aesop;
        exact h_g3_im_in_H _ _ ha ( by solve_by_elim );
    simp_all +decide [ C_coset_g ];
    simp +decide [ ZMod.val, pow_succ, mul_assoc ];
    simp +decide [ ← mul_assoc, H_finset ];
    ext; simp [H_cubes];
    constructor <;> rintro ⟨ a, rfl ⟩;
    · exact ⟨ a * g, by push_cast; ring ⟩;
    · refine' ⟨ a * g⁻¹, _ ⟩ ; group;
      norm_cast ; simp +decide [ pow_succ, mul_assoc, mul_comm, mul_left_comm ]

/-
Lemma: If k is congruent to 2 modulo 3, then the raw periods transform as alpha -> -gamma, beta -> -beta, gamma -> -alpha.
-/
lemma raw_periods_transform_of_pow_two (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) (k : ℕ)
  (hk : k % 3 = 2) :
  alpha_raw_g q (g ^ k) = - gamma_raw_g q g ∧
  beta_raw_g q (g ^ k) = - beta_raw_g q g ∧
  gamma_raw_g q (g ^ k) = - alpha_raw_g q g := by
    unfold alpha_raw_g beta_raw_g gamma_raw_g;
    unfold eta_period_g;
    have := coset_g_eq_of_pow_two q g k hk; aesop;

/-
Lemma: If k is congruent to 1 modulo 3, then the raw periods are unchanged.
-/
lemma raw_periods_transform_of_pow_one (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) (k : ℕ)
  (hk : k % 3 = 1) :
  alpha_raw_g q (g ^ k) = alpha_raw_g q g ∧
  beta_raw_g q (g ^ k) = beta_raw_g q g ∧
  gamma_raw_g q (g ^ k) = gamma_raw_g q g := by
    unfold alpha_raw_g beta_raw_g gamma_raw_g;
    unfold eta_period_g; simp +decide [ coset_g_eq_of_pow_one q g k hk ] ;

/-
Lemma: If k is congruent to 2 modulo 3 and the raw product is ±q, then epsilon flips sign.
-/
lemma epsilon_g_transform_of_pow_two (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) (k : ℕ)
  (hk : k % 3 = 2)
  (h_prod : alpha_raw_g q g * beta_raw_g q g * gamma_raw_g q g = (q : CyclotomicField q ℚ) ∨
            alpha_raw_g q g * beta_raw_g q g * gamma_raw_g q g = -(q : CyclotomicField q ℚ)) :
  epsilon_g q (g ^ k) = - epsilon_g q g := by
    -- By definition of epsilon, we know that if the product of the raw periods for g is q, then epsilon is 1, and if it's -q, then epsilon is -1.
    have h_epsilon_def : epsilon_g q g = if alpha_raw_g q g * beta_raw_g q g * gamma_raw_g q g = (q : CyclotomicField q ℚ) then 1 else -1 := by
      exact?;
    have h_epsilon_def_k : epsilon_g q (g ^ k) = if alpha_raw_g q (g ^ k) * beta_raw_g q (g ^ k) * gamma_raw_g q (g ^ k) = (q : CyclotomicField q ℚ) then 1 else -1 := by
      norm_cast;
    split_ifs at * <;> simp_all +decide;
    · have := raw_periods_transform_of_pow_two q g k hk; simp_all +decide [ mul_assoc ] ;
      simp_all +decide [ mul_assoc, mul_comm, mul_left_comm ];
      rw [ neg_eq_iff_add_eq_zero ] at * ; norm_cast at * ; simp_all +decide [ Nat.Prime.ne_zero Fact.out ];
    · have h_prod_k : alpha_raw_g q (g ^ k) * beta_raw_g q (g ^ k) * gamma_raw_g q (g ^ k) = - (alpha_raw_g q g * beta_raw_g q g * gamma_raw_g q g) := by
        rw [ raw_periods_transform_of_pow_two q g k hk |>.1, raw_periods_transform_of_pow_two q g k hk |>.2.1, raw_periods_transform_of_pow_two q g k hk |>.2.2 ] ; ring;
      grind

/-
Lemma: If k is congruent to 1 modulo 3, then epsilon is unchanged.
-/
lemma epsilon_g_transform_of_pow_one (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) (k : ℕ)
  (hk : k % 3 = 1) :
  epsilon_g q (g ^ k) = epsilon_g q g := by
    -- By `raw_periods_transform_of_pow_one`, the raw periods are identical.
    have h_raw_periods : alpha_raw_g q (g ^ k) = alpha_raw_g q g ∧ beta_raw_g q (g ^ k) = beta_raw_g q g ∧ gamma_raw_g q (g ^ k) = gamma_raw_g q g := by
      exact?;
    unfold epsilon_g; aesop;

/-
Lemma: The automorphism sigma_g maps eta_i(g) to eta_{i+1}(g).
-/
lemma sigma_g_eta_period_g (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) (i : ZMod 3) :
  sigma_cyclotomic_v2 q g (eta_period_g q g i) = eta_period_g q g (i + 1) := by
    unfold eta_period_g;
    have h_bij : (C_coset_g q g i).image (fun x => (g : ZMod q) * x) = C_coset_g q g (i + 1) := by
      ext; simp [C_coset_g, H_finset];
      fin_cases i <;> simp +decide [ pow_succ', mul_assoc ];
      ...
      · simp +decide [ ZMod.val, pow_succ', mul_assoc ];
        ...
        constructor <;> rintro ⟨ a, ha, rfl ⟩;
        · obtain ⟨ b, rfl ⟩ := ha;
          refine' ⟨ _, ⟨ b * g, rfl ⟩, _ ⟩ ; simp +decide [ pow_succ', mul_assoc ];
          ring;
        · obtain ⟨ b, rfl ⟩ := ha;
          refine' ⟨ b ^ 3 / g ^ 3, _, _ ⟩ <;> norm_num;
          · exact ⟨ b / g, by simp +decide [ div_pow ] ⟩;
          · field_simp;
    rw [ ← h_bij, Finset.sum_image ];
    · simp +decide [ map_sum, sigma_zeta_q ];
      simp +decide [ pow_mul, ZMod.val_mul ];
      refine' Finset.sum_congr rfl fun x hx => _;
      rw [ ← pow_mul, ← Nat.mod_add_div ( ( g : ZMod q ).val * x.val ) q, pow_add, pow_mul ] ; norm_num [ zeta_q ];
      rw [ IsCyclotomicExtension.zeta_pow ] ; aesop;
    · intro x hx y hy; aesop;

/-
Lemma: The automorphism sigma_g cycles the raw periods alpha -> beta -> gamma -> alpha.
-/
lemma sigma_g_raw_periods (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) :
  sigma_cyclotomic_v2 q g (alpha_raw_g q g) = beta_raw_g q g ∧
  sigma_cyclotomic_v2 q g (beta_raw_g q g) = gamma_raw_g q g ∧
  sigma_cyclotomic_v2 q g (gamma_raw_g q g) = alpha_raw_g q g := by
    unfold alpha_raw_g beta_raw_g gamma_raw_g;
    -- Apply the lemma that states $\sigma_g(\eta_i) = \eta_{i+1}$ to each term in the sum.
    have h_sigma_eta : ∀ i : ZMod 3, sigma_cyclotomic_v2 q g (eta_period_g q g i) = eta_period_g q g (i + 1) := by
      exact?;
    simp +decide [ h_sigma_eta ];
    exact ⟨ rfl, rfl, rfl ⟩

/-
Lemma: The normalized periods are permuted under a change of generator.
-/
lemma normalized_periods_transform (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) (k : ℕ)
  (hk_not_div_3 : k % 3 ≠ 0)
  (h_prod : alpha_raw_g q g * beta_raw_g q g * gamma_raw_g q g = (q : CyclotomicField q ℚ) ∨
            alpha_raw_g q g * beta_raw_g q g * gamma_raw_g q g = -(q : CyclotomicField q ℚ)) :
  (k % 3 = 1 → alpha_g q (g ^ k) = alpha_g q g ∧ beta_g q (g ^ k) = beta_g q g ∧ gamma_g q (g ^ k) = gamma_g q g) ∧
  (k % 3 = 2 → alpha_g q (g ^ k) = gamma_g q g ∧ beta_g q (g ^ k) = beta_g q g ∧ gamma_g q (g ^ k) = alpha_g q g) := by
    constructor <;> intro hk <;> simp_all +decide [ alpha_g, beta_g, gamma_g, epsilon_g_transform_of_pow_one, epsilon_g_transform_of_pow_two ];
    · have := raw_periods_transform_of_pow_one q g k hk; aesop;
    · have := raw_periods_transform_of_pow_two q g k hk; simp_all +decide [ Algebra.smul_def ] ;

/-
Lemma: The automorphism sigma_g cycles the normalized periods alpha -> beta -> gamma -> alpha.
-/
lemma sigma_g_normalized_periods (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) :
  sigma_cyclotomic_v2 q g (alpha_g q g) = beta_g q g ∧
  sigma_cyclotomic_v2 q g (beta_g q g) = gamma_g q g ∧
  sigma_cyclotomic_v2 q g (gamma_g q g) = alpha_g q g := by
    unfold alpha_g beta_g gamma_g;
    simp +decide [ Algebra.smul_def ];
    exact ⟨ Or.inl <| by simpa using sigma_g_raw_periods q g |>.1, Or.inl <| by simpa using sigma_g_raw_periods q g |>.2.1, Or.inl <| by simpa using sigma_g_raw_periods q g |>.2.2 ⟩

/-
Lemma: If alpha satisfies the period equation, then beta and gamma also satisfy it.
-/
lemma period_equation_propagates (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ)
  (h_alpha : alpha_g q g ^ 3 = (q : CyclotomicField q ℚ) * alpha_g q g + (q : CyclotomicField q ℚ)) :
  beta_g q g ^ 3 = (q : CyclotomicField q ℚ) * beta_g q g + (q : CyclotomicField q ℚ) ∧
  gamma_g q g ^ 3 = (q : CyclotomicField q ℚ) * gamma_g q g + (q : CyclotomicField q ℚ) := by
    have h_beta : beta_g q g ^ 3 = (q : CyclotomicField q ℚ) * beta_g q g + (q : CyclotomicField q ℚ) := by
      have := sigma_g_normalized_periods q g;
      replace h_alpha := congr_arg ( sigma_cyclotomic_v2 q g ) h_alpha; aesop;
    have h_gamma : gamma_g q g ^ 3 = (q : CyclotomicField q ℚ) * gamma_g q g + (q : CyclotomicField q ℚ) := by
      have := sigma_g_normalized_periods q g;
      replace h_beta := congr_arg ( sigma_cyclotomic_v2 q g ) h_beta ; simp_all +decide [ pow_succ ] ;
    exact ⟨h_beta, h_gamma⟩

/-
Lemma: The sum of the raw periods is 0.
-/
lemma sum_raw_periods_eq_zero (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) :
  alpha_raw_g q g + beta_raw_g q g + gamma_raw_g q g = 0 := by
    unfold alpha_raw_g beta_raw_g gamma_raw_g;
    ring

/-
Lemma: If alpha(g) satisfies the period equation, then the product of raw periods is ±q.
-/
lemma raw_product_from_assumption (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ)
  (h_alpha : alpha_g q g ^ 3 = (q : CyclotomicField q ℚ) * alpha_g q g + (q : CyclotomicField q ℚ)) :
  alpha_raw_g q g * beta_raw_g q g * gamma_raw_g q g = (q : CyclotomicField q ℚ) ∨
  alpha_raw_g q g * beta_raw_g q g * gamma_raw_g q g = -(q : CyclotomicField q ℚ) := by
    have h_prod : (alpha_g q g) * (beta_g q g) * (gamma_g q g) = q := by
      have h_beta_gamma : beta_g q g ^ 3 = (q : CyclotomicField q ℚ) * beta_g q g + (q : CyclotomicField q ℚ) ∧ gamma_g q g ^ 3 = (q : CyclotomicField q ℚ) * gamma_g q g + (q : CyclotomicField q ℚ) := by
        exact?;
      have h_sum : alpha_g q g + beta_g q g + gamma_g q g = 0 := by
        unfold alpha_g beta_g gamma_g;
        rw [ ← smul_add, ← smul_add ];
        rw [ sum_raw_periods_eq_zero ] ; norm_num;
      grind +ring
    generalize_proofs at *;
    unfold alpha_g beta_g gamma_g at h_prod; simp_all +decide [ mul_assoc, mul_left_comm, mul_comm ] ;
    unfold epsilon_g at *; simp_all +decide [ ← mul_assoc ] ;
    exact Classical.or_iff_not_imp_left.2 fun h => by linear_combination -h_prod h;

/-
Theorem: The period equation is generator-independent. If alpha(g) satisfies it, then alpha(g'), beta(g'), and gamma(g') satisfy it for any other generator g'.
-/
theorem generator_independence (q : ℕ) [Fact (Nat.Prime q)] (h_div : 3 ∣ q - 1)
  (g g' : (ZMod q)ˣ) (hg : orderOf g = q - 1) (hg' : orderOf g' = q - 1)
  (h_alpha : alpha_g q g ^ 3 = (q : CyclotomicField q ℚ) * alpha_g q g + (q : CyclotomicField q ℚ)) :
  alpha_g q g' ^ 3 = (q : CyclotomicField q ℚ) * alpha_g q g' + (q : CyclotomicField q ℚ) ∧
  beta_g q g' ^ 3 = (q : CyclotomicField q ℚ) * beta_g q g' + (q : CyclotomicField q ℚ) ∧
  gamma_g q g' ^ 3 = (q : CyclotomicField q ℚ) * gamma_g q g' + (q : CyclotomicField q ℚ) := by
    obtain ⟨ k, hk ⟩ := cosets_change q h_div g g' hg hg';
    -- By `raw_product_from_assumption`, the raw product condition for `normalized_periods_transform` is satisfied.
    have h_raw_product : alpha_raw_g q g * beta_raw_g q g * gamma_raw_g q g = (q : CyclotomicField q ℚ) ∨ alpha_raw_g q g * beta_raw_g q g * gamma_raw_g q g = -(q : CyclotomicField q ℚ) := by
      exact?;
    cases' hk.2 with hk1 hk2;
    · have h_normalized_periods : alpha_g q (g ^ k) = alpha_g q g ∧ beta_g q (g ^ k) = beta_g q g ∧ gamma_g q (g ^ k) = gamma_g q g := by
        apply normalized_periods_transform q g k (by
        linarith) (by
        exact h_raw_product) |>.1 hk1;
      have := period_equation_propagates q g h_alpha; aesop;
    · -- By `normalized_periods_transform`, we have `alpha_g q (g ^ k) = gamma_g q g`, `beta_g q (g ^ k) = beta_g q g`, and `gamma_g q (g ^ k) = alpha_g q g`.
      have h_transformed : alpha_g q (g ^ k) = gamma_g q g ∧ beta_g q (g ^ k) = beta_g q g ∧ gamma_g q (g ^ k) = alpha_g q g := by
        exact normalized_periods_transform q g k ( by aesop ) ( by aesop ) |>.2 hk2;
      have := period_equation_propagates q g h_alpha; aesop;

/-
Lemma: The fixed-generator period eta_period is equal to the generator-parameterized period eta_period_g with g = g_gen q.
-/
lemma eta_period_eq_eta_period_g (q : ℕ) [Fact (Nat.Prime q)] (i : ZMod 3) :
  eta_period q i = eta_period_g q (g_gen q) i := by
    convert rfl

/-
Lemma: The fixed-generator raw period difference alpha_cyclotomic is equal to the generator-parameterized alpha_raw_g with g = g_gen q.
-/
lemma alpha_cyclotomic_eq_alpha_raw_g (q : ℕ) [Fact (Nat.Prime q)] :
  alpha_cyclotomic q = alpha_raw_g q (g_gen q) := by
    -- By definition, `alpha_cyclotomic` is `eta_period q 0 - eta_period q 1`.
    -- By definition, `alpha_raw_g` is `eta_period_g q g 0 - eta_period_g q g 1`.
    -- Using `eta_period_eq_eta_period_g`, these are equal when `g = g_gen q`.
    simp [alpha_cyclotomic, alpha_raw_g, eta_period_eq_eta_period_g]

/-
Lemma: The fixed-generator normalization factor epsilon_cyclotomic is equal to the generator-parameterized epsilon_g with g = g_gen q.
-/
lemma epsilon_cyclotomic_eq_epsilon_g (q : ℕ) [Fact (Nat.Prime q)] :
  epsilon_cyclotomic q = epsilon_g q (g_gen q) := by
    exact?

/-
Lemma: The fixed-generator normalized period alpha_star is equal to the generator-parameterized alpha_g with g = g_gen q.
-/
lemma alpha_star_eq_alpha_g (q : ℕ) [Fact (Nat.Prime q)] :
  alpha_star q = alpha_g q (g_gen q) := by
    congr! 2

/-
Lemma: The fixed-generator normalized period beta_star is equal to the generator-parameterized beta_g with g = g_gen q.
-/
lemma beta_star_eq_beta_g (q : ℕ) [Fact (Nat.Prime q)] :
  beta_star q = beta_g q (g_gen q) := by
    unfold beta_star beta_g; aesop;

/-
Lemma: The fixed-generator normalized period gamma_star is equal to the generator-parameterized gamma_g with g = g_gen q.
-/
lemma gamma_star_eq_gamma_g (q : ℕ) [Fact (Nat.Prime q)] :
  gamma_star q = gamma_g q (g_gen q) := by
    unfold gamma_star gamma_g;
    unfold gamma_cyclotomic gamma_raw_g; rw [ epsilon_cyclotomic_eq_epsilon_g ] ;
    rw [ eta_period_eq_eta_period_g, eta_period_eq_eta_period_g ]

/-
Theorem: If the fixed alpha_star satisfies the period equation, then alpha_g satisfies it for any generator g.
-/
theorem period_equation_universal (q : ℕ) [Fact (Nat.Prime q)] (h_div : 3 ∣ q - 1)
  (h_alpha_star : alpha_star q ^ 3 = (q : CyclotomicField q ℚ) * alpha_star q + (q : CyclotomicField q ℚ))
  (g : (ZMod q)ˣ) (hg : orderOf g = q - 1) :
  alpha_g q g ^ 3 = (q : CyclotomicField q ℚ) * alpha_g q g + (q : CyclotomicField q ℚ) := by
    convert generator_independence q h_div ( g_gen q ) g _ hg _ |> And.left using 1;
    · rw [ orderOf_eq_card_of_forall_mem_zpowers ] <;> norm_num [ g_gen ];
      · exact Nat.totient_prime Fact.out;
      · exact Classical.choose_spec ( IsCyclic.exists_generator ( α := ( ZMod q ) ˣ ) );
    · convert h_alpha_star using 1

/-
Theorem: If the raw alpha satisfies the +q equation, then for a generator change with k=2 mod 3, the new raw alpha satisfies the -q equation.
-/
theorem raw_period_equation_flip (q : ℕ) [Fact (Nat.Prime q)] (g : (ZMod q)ˣ) (k : ℕ)
  (hk : k % 3 = 2)
  (h_alpha_raw : alpha_raw_g q g ^ 3 = (q : CyclotomicField q ℚ) * alpha_raw_g q g + (q : CyclotomicField q ℚ)) :
  alpha_raw_g q (g ^ k) ^ 3 = (q : CyclotomicField q ℚ) * alpha_raw_g q (g ^ k) - (q : CyclotomicField q ℚ) := by
    -- By `raw_periods_transform_of_pow_two`, $\alpha_{\mathrm{raw}}(g^k) = -\gamma_{\mathrm{raw}}(g)$.
    have h_alpha_raw_g_k : alpha_raw_g q (g ^ k) = -gamma_raw_g q g := by
      exact raw_periods_transform_of_pow_two q g k hk |>.1;
    -- By `sigma_g_raw_periods`, $\gamma_{\mathrm{raw}}(g)^3 = q\gamma_{\mathrm{raw}}(g) + q$.
    have h_gamma_raw_g : gamma_raw_g q g ^ 3 = (q : CyclotomicField q ℚ) * gamma_raw_g q g + (q : CyclotomicField q ℚ) := by
      have h_gamma_raw_g : sigma_cyclotomic_v2 q g (sigma_cyclotomic_v2 q g (alpha_raw_g q g)) = gamma_raw_g q g := by
        rw [ sigma_g_raw_periods q g |>.1, sigma_g_raw_periods q g |>.2.1 ];
      have h_gamma_raw_g : (sigma_cyclotomic_v2 q g) ((sigma_cyclotomic_v2 q g) (alpha_raw_g q g)) ^ 3 = (sigma_cyclotomic_v2 q g) ((sigma_cyclotomic_v2 q g) (alpha_raw_g q g ^ 3)) := by
        simp +decide [ pow_succ ];
      aesop;
    rw [ h_alpha_raw_g_k ] ; linear_combination' -h_gamma_raw_g;